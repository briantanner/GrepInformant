...
let url = "https://%s.grepolis.com/data/%s.txt.gz";
...
class Grepolis {
  ...
  getData(server, endpoint) {
    ...
    this.servers[server][endpoint] = this.servers[server][endpoint] || false;
    ...
    return new Promise((resolve, reject) => {
      ...
      http.get(_url, res => {
        if (res.headers['content-type'] !== 'application/octet-stream') {
          return reject('Data file not found.');
        }
        ...
        // Retry 3x before rejection
        if (this.servers[server][endpoint] <= 3) {
          this.servers[server][endpoint]++;
          
          logger.warn("Retrying %s [%s]... %d", server, endpoint, this.servers[server][endpoint]);

          this.getData(server, endpoint)
          .then(results => {
            this.servers[server][endpoint] = 0;
