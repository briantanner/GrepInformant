...
class Importer {
  ...
  addConquerNames() {
    return new Promise(resolve => {
      this.getDatabaseData(['players', 'alliances', 'conquers'])
   ...
   * @return {Promise} Promise
   */
  getDatabaseData(keys, time) {
    return new Promise((resolve, reject) => {
      let calls = [],
        queries = {
          players: escape("select * from players where server = %L and deleted != true order by rank asc", this.server),
          alliances: escape("select * from alliances where server = %L and deleted != true order by rank asc", this.server),
          towns: escape("select * from towns where server = %L and deleted != true", this.server),
          conquers: escape("select * from conquers where server = %L", this.server),
          all_alliances: escape("select * from alliances where server = %L and deleted != true order by rank asc", this.server)
        };

      if (time) {
        queries = {
          players: escape("select * from player_daily where server = %L and time > %s order by rank asc", this.server, time),
          alliances: escape("select * from alliance_daily where server = %L and time > %s order by rank asc", this.server, time),
          all_alliances: escape("select * from alliances where server = %L and deleted != true order by rank asc", this.server, time)
        };
      }
      ...
      models.Conquers.findAll({
        ...
        let updateQueue = async.queue(function (task, callback) {
          ...
          task.conquer.updateAttributes(task.update).then(() => {
            this.info(format('updated {}', task.conquer.id));
            return callback();
          }).catch(err => {
            this.error(err);
            return callback();
        ...
        conquers.forEach(conquer => {
          ...
          if (conquer.newally_name !== null ||
              conquer.oldally_name !== null ||
              conquer.oldplayer_name !== null) {
                this.info(format('skipping {}', conquer.id));
                return;
              }
          ...
          updateQueue.push({ conquer, update });
        });
