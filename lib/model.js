...
Model.alliances = function (server, options, callback) {
  var query = options.query || util.format("select * from alliances where server = '%s' and deleted != true", server);
...
Model.playerUpdates = function (server, table, options, callback) {
  table = (table === 'hourly') ? 'player_updates' : util.format("player_%s", table);

  var query = util.format("select * from %s where server = '%s'", table, server);
